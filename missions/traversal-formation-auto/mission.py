...
from constants import INITIAL_VERTICAL_SEPARATION
...
class FormationTraversalAutoMission(Mission):

    port = 4002
    mission_id = 'formation-traversal-auto'
    drone_id = util.uuid()
    ...
    @callback(
        ...
        required_params=('coordinates', 'initial_alt', 'alt', 'hover_duration', 'speed', 'target_offset', 'center_location'),
        public=False,
    )
    def launch(self, data, *args, **kwargs):
        ...
        :param data: Required to be of the form:
            {
                ...
                'initial_alt': 0.00 # Altitude that the drones will take off to and form the formation with
                'alt': 0.00 # Altitude that this drone will fly at after assuming formation
                'hover_duration' : 0.00,
                'speed': 0.00,
                'target_offset': {
                    'bearing': 0.00,
        ...
        initial_altitude = data['initial_alt']
        altitude = data['alt']
        speed = data['speed']
        self.formation_offset = FormationOffset.deserialize(data['target_offset'])
        center_location = Coordinate.deserialize(data['center_location'])
        self.log.debug('Beginning launch with initial_alt: {init_alt}, final_alt: {alt}, hover_duration: {hover_duration}, speed: {speed}'.format(
                init_alt=initial_altitude,
                alt=altitude,
        ...
        self.log.debug('Taking off to altitude: {alt}'.format(alt=initial_altitude))
        self.dc.take_off(initial_altitude)

        self.log.debug('Take off complete. Moving to formation position')
        self.visit_with_offset(center_location, initial_altitude, speed)
        self.log.debug('Moved into formation position. Waiting for others')
        self.broadcast_completion()
        self.wait_for_others()

        self.log.debug('All drones in formation position. Moving to altitude: {alt}'.format(alt=altitude))
        self.visit_with_offset(center_location, altitude, speed)
        self.log.debug('Formation position assumed. Waiting for others.')
        self.broadcast_completion()
        self.wait_for_others()
    ...
    def launch_drones(self, data, center_location):
        ...
        initial_altitude = data['min_alt'] + INITIAL_VERTICAL_SEPARATION * len(self.all_drones)
        coordinates = self.make_serialized_coordinates(
            data['region_name'],
            center_location)
        formation = DroneFormation(
            drone_count=len(self.all_drones),
            radius=FORMATION_RADIUS)

        for i, (drone_ip, drone_port) in enumerate(self.all_drones):
            altitude -= VERTICAL_SEPARATION # Separate each drone altitude by VERTICAL_SEPARATION
            initial_altitude -= INITIAL_VERTICAL_SEPARATION # Initially separate drones by the initial vertical separation
            launch_data = {
                'coordinates': coordinates,
                'alt': altitude,
                'initial_alt': initial_altitude,
                'hover_duration': data['hover_duration'],
                'speed': data['speed'],
                'target_offset': formation.get_offset(i).serialize(),
                'center_location': center_location.serialize(),
            }
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default='path-log_formation-traversal-auto_{year}_{month}_{day}_{hour}_{min}_{sec}'.format(
